...
class Cache:
    ...
    def __init_cache_file_location(self):
        ...
        if self.__curr_system_type == "OSX":
            self.curr_config_file = self.__cache_file_osx
        elif self.__curr_system_type == "Windows":
            self.curr_config_file = self.__cache_file_windows_tpl.format(systemRoot=self.__curr_windows_system_root)
        elif self.__curr_system_type == "Linux":
            self.curr_config_file = self.__cache_file_linux

    def load_config_file(self):
        """
        初始化缓存目录, 不存在则创建
        """
        if not os.path.exists(self.curr_config_file):
            os.makedirs(self.curr_config_file)
    ...
    @staticmethod
    def get_file_by_app(file_id: str) -> str | None:
        """
